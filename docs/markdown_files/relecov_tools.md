...
Please find more information about the tools in their [**GitHub page**](https://github.com/BU-ISCIII/relecov-tools).

**relecov-tools** relecov-tools is a collection of helper tools for assembling the various components of the **RELECOV** platform (Spanish Network for Genomic Surveillance of SARS-CoV-2), including data download, processing, validation, and upload to public databases, as well as running analyses and storing results in a database.
...
- [relecov-tools](#relecov-tools)
  ...
  - [Modules](#modules)
      ...
      - [`read-bioinfo-metadata`](#read-bioinfo-metadata)
        - [Configuration of the module `read-bioinfo-metadata`](#configuration-of-the-module-read-bioinfo-metadata)
      - [`validate`](#validate)
      - [`map`](#map)
...
- Python > 3.8
- [7-Zip](https://www.7-zip.org/) (via the `p7zip` package) to handle compressed files. Please make sure this is installed before running the code.
...
If you already have conda installed, please do the following:
...
relecov-tools is available in [pypi.org](https://pypi.org/project/relecov-tools/) and can be installed via `pip`:
...
If you want to install the latest code in the repository, you can run the following commands:
...
- `--hex-code`: By default all files generated will include a date and a unique hexadecimal code which is randomly generated upon execution. By using this argument, you can pre-define the resulting hexadecimal code. NOTE: Keep in mind that this could overwrite existing files.

## Modules

#### `download`

The command `download` connects to a transfer protocol (currently SFTP) and downloads all files in the different available folders with the passed credentials. In addition, it checks whether the files in the current folder match the files indicated in the metadata file, apart from checking if there are md5sum hashes for each file as well. If not, it creates one before storing the corresponding file in the final repository.
...
- If no `config_file` is specified, default values are loaded from `conf/configuration.json`, and the user will be prompted to enter their username and password.
- Default values can be overridden using a YAML configuration file, where you can specify values such as `user`, `password`, `sftp_server`, and more.
...
`read-lab-metadata` module reads the excel file with laboratory metadata and processes it including additional necessary fields.
...
An example of the metadata excel file can be found [**here**](./relecov_tools/example_data/METADATA_LAB_TEST.xlsx).

#### `send-mail`

`send-mail` sends a validation summary report by email using predefined Jinja templates. It supports attachments, multiple recipients, and includes the option to add additional notes manually or from a `.txt` file.
...
`read-bioinfo-metadata` module includes the results from the Bioinformatics analysis into the JSON file previously created by means of the `read-lab-metadata` module.
...
##### Configuration of the module `read-bioinfo-metadata`
...
`validate` module validates the data in JSON format outputted by the `read-lab-metadata` module against a JSON schema, in this case the [**Relecov schema specification**](./relecov_tools/schema/relecov_schema.json). It also creates a summary of the errors and warnings found, presented in Excel format as a report for users.
...
The module `map` converts data in JSON format from the Relecov data model to the ENA or GISAID data model using their respective schemas, according to their annotated ontology terms.
...
The `upload-to-ena` module uses JSON data mapped to the ENA schema to upload raw data and metadata to the ENA database using the [**ena_upload_cli**](https://github.com/usegalaxy-eu/ena-upload-cli) package.
...
The `upload-to-gisaid` command uses JSON data mapped to the GISAID schema to upload raw data and metadata to the GISAID database.
...
This module uploads the information contained in the final JSON files (from the `read-bioinfo-metadata` module) to the database.
...
This module creates the folder structure to execute a given pipeline for the latest sample batches after running the `download`, read-lab-metadata and `validate` modules. This module creates symbolic links for each sample and generates the necessary files for pipeline execution using information from `validated_BATCH-NAME_DATE.json`.
...
This module executes the `download`, `read-lab-metadata` and `validate` modules sequentially using a config file to provide arguments for each one of them. It also creates a global report with all logs from the three processes in a user-friendly `.xlsx` format. The config file should include the name of each module to execute along with the necessary parameters in YAML format.
...
This module generates an `.xlsx` file with all entries found for a specified laboratory in a given set of `log_summary.json` files (from the `log-summary` module). The laboratory name must match one of the keys in the provided logs for it to work.
...
This module is used to create an additional config file that will override the configuration indicated in `conf/configuration.json`. You may provide this configuration in a YAML or JSON file. If you want the keys in your additional configuration to be grouped under a specific key name, please use the parameter `-n, --config_name`. Otherwise, the file content will be parsed with no additional processing.
...
The *--log-path* option, given via the command-line interface (CLI), specifies the destination directory where logs will be saved during execution. When provided, it overrides the default logging behavior and directs all log files to the specified folder.

**How default logs are generated (when `--log-path` is not used):**  
A predefined default location is used, found in `configuration.json` under the `logs_config` key.  
- If the executed module is found in the `modules_outpath` subkey, the log will be generated in the specified folder.  
- Otherwise, it will be generated in `default_outpath/module`.

If you want your logs to be sent to custom locations depending on the executed module, you can do so by using `add-extra-config`, providing custom settings.

#### Custom logs

After executing each module, you may find a custom log report in JSON format named `EXECUTED-MODULE_<date>_hex_log_summary.json`.These custom log summaries can be useful to detect errors in metadata for fixing them and/or notifying users.
...
Proper documentation about this will be available soon!

## Acknowledgements

The idea and design of this Python package are heavily inspired by [nf-core/tools](https://github.com/nf-core/tools).