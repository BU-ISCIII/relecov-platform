...
compose_run config >/dev/null
    ...
    container_id="$(resolve_service_container iskylims_app)"
    [ -n "$container_id" ] || fail "Service iskylims_app has no container"
    ensure_service_running iskylims_app "$container_id" >/dev/null
    engine_exec exec "$container_id" bash -lc 'cd "$INSTALL_PATH" && source virtualenv/bin/activate && python manage.py check && ! python manage.py showmigrations --plan | grep -F '"'"'[ ]'"'"''
    echo "PASS: iskylims_app Django checks and migrations"
check_url() {
