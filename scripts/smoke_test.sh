...
compose_run config >/dev/null
    container_id="$(resolve_service_container relecov-platform)"
    [ -n "$container_id" ] || fail "Service relecov-platform has no container"
    ensure_service_running relecov-platform "$container_id" >/dev/null
    ...
    echo "PASS: relecov-platform Django checks and migrations"
...
echo "RELECOV Platform deployment smoke test passed."
